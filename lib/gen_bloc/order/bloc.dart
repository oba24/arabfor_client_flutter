import 'package:saudimarchclient/gen_bloc/order/model.dart';
...
class OrderBloc extends Bloc<OrderEvent, OrderState> {
  ...
  Stream<OrderState> mapEventToState(OrderEvent event) async* {
    if (event is StartOrderEvent) {
      ...
      CustomResponse response = await allRepo();
      if (response.success) {
        OrderModel _model = OrderModel.fromJson(response.response?.data);
        yield DoneOrderState(response.msg, _model.data);
      } else {
        yield FaildOrderState(msg: response.msg, errType: response.errType!, statusCode: response.statusCode);
      }
    }
  ...
  Future<CustomResponse> allRepo() async {
    CustomResponse response = await serverGate.getFromServer(url: "client/client_order");
    return response;
  }

  Future<CustomResponse> singleRepo(int id) async {
    CustomResponse response = await serverGate.getFromServer(url: "client/client_order/$id");
    return response;
  }
