...
class CustomImagePicker {
  static openGallery(Function(List<File> fils) onGet, {bool isFile = false}) async {
    List<File> _fils = [];
    if (isFile) {
  ...
  static dynamic openImagePicker(
      {required Function(File file) onSubmit, Function(List<File> fils)? onMuiltySubmit, bool withCrop = false, bool withFile = false}) {
      ...
      builder: (BuildContext context) {
        return CupertinoActionSheet(
          cancelButton: CupertinoButton(
            child: Text(LocaleKeys.Cancel.tr(), style: context.textTheme.subtitle1),
          ...
          actions: <Widget>[
            CupertinoButton(
              child: Row(
                children: <Widget>[
                  Icon(CupertinoIcons.photo_camera_solid, color: context.color.secondary),
                  const SizedBox(width: 20),
                  Text(LocaleKeys.Camera.tr(), style: context.textTheme.subtitle1),
                ],
              ),
              onPressed: () async {
                XFile? pickedFile = await _pick.pickImage(source: ImageSource.camera, maxWidth: 400);
            ...
            CupertinoButton(
              child: Row(
                children: <Widget>[
                  ...
                  const SizedBox(width: 20),
                  Text(LocaleKeys.Gallery.tr(), style: context.textTheme.subtitle1),
                ],
              ),
              onPressed: () async {
                XFile? pickedFile = await _pick.pickImage(source: ImageSource.gallery, maxWidth: 400);
                File? _filePicked = File(pickedFile!.path);
                if (withCrop == true) {
                  ImageCropper cropper = ImageCropper();
                  _filePicked = await cropper.cropImage(
